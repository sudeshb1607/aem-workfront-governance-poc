...
/**
 * Converts every {@code *.csv} in the converter service's input folder into a JSON
 * dataset on a configurable schedule (default nightly). Each CSV is processed in
 * isolation with in-run retries (attempts with a backoff pause), so one failing
 * file never blocks the others; anything still failing is retried on the next run.
 */
...
public class WorkfrontJsonConverterScheduler implements Runnable {
    ...
    @ObjectClassDefinition(
            ...
            description = "Converts Workfront dashboard CSV files into JSON datasets on a schedule.")
    ...
    protected void activate(final Config config) {
        ...
        LOG.info("WorkfrontJsonConverterScheduler activated. maxAttempts={}, retryBackoffSeconds={}, pauseBetweenFilesSeconds={}",
    ...
    public void run() {
        ...
        try (ResourceResolver resolver = getServiceResolver()) {
            final String inputFolder = converterService.getInputFolder();
            final Resource folder = resolver.getResource(inputFolder);
            if (folder == null) {
            ...
            for (final Resource child : folder.getChildren()) {
                ...
                // Cool-down before every file except the first.
    ...
    /**
     * Converts one CSV with up to {@code maxAttempts} attempts and a backoff pause
     * between them. Failures are isolated to this file.
     *
     * @return {@code true} if the file converted successfully
     */
    private boolean convertWithRetry(final Resource csvAsset) {
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                final ConversionResult result = converterService.convert(csvAsset);
                if (result.isSuccess()) {
                    return true;
                }
                ...
                LOG.warn("Conversion attempt {}/{} threw for {}",
                        attempt, maxAttempts, csvAsset.getPath(), e);
            }
